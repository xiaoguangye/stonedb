  ...
  for (uint i = 0; i < cond.Size(); i++) {
    ...
    if (cond[i].IsType_JoinSimple() && cond[i].op == common::Operator::O_EQ) {
      if (first_found) {
        ...
        if (dims1.Intersects(cond[i].right_dims)) dims1.Plus(cond[i].right_dims);
  ...
  */
  ...
  for (int i = 0; i < mind->NoDimensions(); i++) {
    if (traversed_dims_[i] && !(tips.count_only && !other_cond_exist_)) {  // storing tuples may be omitted if
      ...
      if (mind->OrigSize(i) > 0x000000007FFFFF00) bin_index_size = 8;
...
void ParallelHashJoiner::ExecuteJoin() {
  ...
  if (traversed_dims_size > 0 && matched_dims_size > 0) {
    ...
    if (watch_traversed_) outer_tuples_ += SubmitOuterTraversed();
  }

  int64_t outer_tuples_matched = 0;
  if (watch_matched_ && !outer_matched_filter_->IsEmpty()) outer_tuples_matched = SubmitOuterMatched(match_mit);
...
int64_t ParallelHashJoiner::TraverseDim(MIIterator &mit, int64_t *outer_tuples) {
  ...
  rccontrol.lock(m_conn->GetThreadID()) << "Begin traversed with " << traversed_fragment_count << " threads with "
  ...
  try {
    for (MITaskIterator *iter : task_iterators) {
      ...
      res.insert(rceng->query_thread_pool.add_task(&ParallelHashJoiner::AsyncTraverseDim, this, &params));
    }
  ...
  rccontrol.lock(m_conn->GetThreadID()) << "End traversed " << traversed_rows << "/" << rows_count << " rows."
                                        << system::unlock;

  for (auto &params : traverse_task_params) {
    if (params.too_many_conflicts && !tianmu_sysvar_join_disable_switch_side) {
      if (!force_switching_sides_ && !too_many_conflicts_) too_many_conflicts_ = true;
    }

    if (params.no_space_left)
      rccontrol.lock(m_conn->GetThreadID()) << "No space left of hash table. " << system::unlock;
...
int64_t ParallelHashJoiner::AsyncTraverseDim(TraverseTaskParams *params) {
  ...
  while (params->task_miter->IsValid()) {
    ...
    if (!omit_this_row) {  // else go to the next row - equality cannot be
      ...
      if (watch_traversed_) params->traversed_hash_table->outer_filter()->Set(hash_row);
...
int64_t ParallelHashJoiner::MatchDim(MIIterator &mit) {
  ...
  rccontrol.lock(m_conn->GetThreadID()) << "Begin match dim of " << rows_count << " rows, spliting into "
  ...
  if (task_iterators.size() > 1) {
    ...
    try {
      for (MITaskIterator *iter : task_iterators) {
        ...
        res.insert(rceng->query_thread_pool.add_task(&ParallelHashJoiner::AsyncMatchDim, this, &params));
      }
    } catch (std::exception &e) {
      res.get_all_with_except();
      throw e;
  ...
  rccontrol.lock(m_conn->GetThreadID()) << "End match dim. Produced tuples:" << matched_rows << "/" << rows_count
                                        << system::unlock;
...
int64_t ParallelHashJoiner::AsyncMatchDim(MatchTaskParams *params) {
  ...
  if (watch_matched_) outer_matched_filter_->Commit(true);  // Commit the delayed resetsC.

  params->build_item->Finish();

  if (outer_nulls_only_) joined_tuples = 0;  // outer tuples added later
...
void ParallelHashJoiner::InitOuter(Condition &cond) {
  ...
  if (!outer_dims.IsEmpty()) {
    ...
    for (int j = 0; j < outer_dims.Size(); j++)
      if (outer_dims[j] && tips.null_only[j] == false) outer_nulls_only_ = false;
  }
}
...
int64_t ParallelHashJoiner::SubmitOuterMatched(MIIterator &miter) {
  ...
  try {
    for (MITaskIterator *iter : task_iterators) {
      ...
      res.insert(rceng->query_thread_pool.add_task(&ParallelHashJoiner::AsyncSubmitOuterMatched, this, &params,
